...
var goi = setInterval(function() {
	...
	// TODO: get rid of this in favor of resPool
	// parsing data structure
	$(".resourceRow").each(function(index) { 
		var name = $(this).find('.resource-name').text().replace(':','');
		var amount = $(this).find('.resAmount').text();
		var isFull = $(this).find('.resAmount').hasClass('resLimitNotice');
	...
	if (!gamePage.resPool.resourceMap.coal.unlocked) {
		...
		if (k.coal.isFull || k.iron.isFull) {
